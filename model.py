...
class MLP_Final(nn.Module):
    ...
    def __init__(self, in_size, out_size, config):
        super().__init__()
        self.c_fc    = nn.Linear(in_size, in_size//2, bias=config.bias)
        self.c_proj  = nn.Linear(in_size//2, out_size, bias=False)
        self.dropout = nn.Dropout(config.dropout)
    ...
    def __init__(self, config):
        ...
        if self.config.mean_or_flatten == 'mean':
            ...
            if self.config.n_linear_layers > 1:
                self.lm_head = MLP_Final(config.n_embd, config.vocab_size, config)
            else:
